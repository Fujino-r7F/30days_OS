...
unsigned int mamman_total(struct MEMMAN *man);
...
void HariMain(void)
{
	...
	memman_init(memman);
	memman_free(memman, 0x00001000, 0x0009e000); /* 0x0001000 - 0x0009efff */
...
#define ELAGS_AC_BIT	0x00040000
...
unsigned int memtest(unsigned int start, unsigned int end)
{
	...
	/* 386か486移行なのかの確認 */
	eflg = io_load_eflags();
	eflg |= ELAGS_AC_BIT; /* AC-bit = 1 */
	io_store_eflags(eflg);
	eflg = io_load_eflags();
	if ((eflg & ELAGS_AC_BIT) != 0) { /* 386ではAC=1にしても自動で0に戻ってしまう */
		flg486 = 1;
	}
	eflg &= ~ELAGS_AC_BIT; /* AC-bit = 0 */
	io_store_eflags(eflg);
